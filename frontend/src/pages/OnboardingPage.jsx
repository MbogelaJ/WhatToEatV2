...
export default function OnboardingPage() {
  ...
  const [showSignIn, setShowSignIn] = useState(false); // Toggle for returning users
  ...
  // Handle Sign In for returning users - skip onboarding, go to home
  const handleSignIn = async () => {
    if (validateStep2()) {
      ...
      try {
        ...
        if (result.success) {
          navigate('/'); // Go directly to home, skip onboarding
        } else {
          setErrors({ auth: result.error || 'Invalid email or password' });
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white">
      <div className="max-w-lg mx-auto px-4 py-8">
          ...
          <div className="space-y-6" data-testid="step-auth">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
              ...
              <p className="text-sm text-stone-500 mb-6 text-center">
                {showSignIn ? 'Sign in to access your saved preferences' : 'Sign up to save your preferences'}
              </p>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder={showSignIn ? 'Enter your password' : 'Create a password (min 6 chars)'}
                  className="w-full px-4 py-3 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              ...
              {/* Sign In Button (only shown in sign-in mode) */}
              {showSignIn && (
                <button
                  onClick={handleSignIn}
                  disabled={isLoading}
                  className="w-full py-3 px-4 bg-emerald-600 rounded-xl text-white font-medium hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 mb-4"
              ...
              <p className="text-sm text-center text-stone-600 mt-4">
                {showSignIn ? (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => { setShowSignIn(false); setErrors({}); }}
                  ...
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => { setShowSignIn(true); setErrors({}); }}
                      className="text-emerald-600 font-medium hover:underline"
              ...
              {!showSignIn && (
                <>
                  ...
                  <div className="flex items-center justify-center gap-4">
                    ...
                    <button type="button" className="w-12 h-12 flex items-center justify-center bg-black text-white rounded-xl hover:bg-stone-800 transition-colors opacity-50 cursor-not-allowed" data-testid="apple-login-btn" disabled title="Coming soon">
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
                      </svg>
                    </button>
                    <button type="button" className="w-12 h-12 flex items-center justify-center bg-[#1877F2] text-white rounded-xl opacity-50 cursor-not-allowed" data-testid="facebook-login-btn" disabled title="Coming soon">
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                      </svg>
                    </button>
                    <button type="button" className="w-12 h-12 flex items-center justify-center bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737] text-white rounded-xl opacity-50 cursor-not-allowed" data-testid="instagram-login-btn" disabled title="Coming soon">
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="flex gap-3 mt-8">
          ...
          {/* Hide Continue button when in Sign In mode at Step 2 */}
          {!(step === 2 && showSignIn) && (
            <button
              onClick={handleNext}
              disabled={isLoading}
